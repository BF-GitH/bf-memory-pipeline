...
export async function bulkEmbedAllFacts(onProgress = () => {}) {
    ...
    for (const [category, db] of Object.entries(databases)) {
        for (const fact of (db.facts || [])) {
            if (!isActiveFact(fact) || isSequenceFact(fact) || Array.isArray(fact.embedding)) continue;
            todo.push(fact);
            byCat.set(category, db);
        }
    }
    const total = todo.length;
    if (total === 0) return { succeeded: 0, total: 0 };
    addDebugLog('info', `Bulk embed: ${total} fact(s) need vectors`);

    let succeeded = 0;
    for (let i = 0; i < todo.length; i += EMBED_BATCH_SIZE) {
        succeeded += await embedFacts(todo.slice(i, i + EMBED_BATCH_SIZE), {});
        onProgress({ done: Math.min(i + EMBED_BATCH_SIZE, total), total });
    }
    for (const db of byCat.values()) { try { await saveDatabase(db); } catch (e) { addDebugLog('fail', `Bulk embed save failed: ${e.message || e}`); } }
    addDebugLog('pass', `Bulk embed done: ${succeeded}/${total} embedded`);
